...
const text = `Your guest with booking number ${updatedGuest.bookingNumber} has been arrived!`;
...
const emailSubject = "Your guest has been arrived!";
const emailBody = `Your guest with booking number ${updatedGuest.bookingNumber} has been arrived!`;
  ...
  try {
    ...
    if (inOrOut == "in") {
      const updatedTimeIn = await Guest.findOneAndUpdate(
        ...
        { $push: { timeArrived: { date: datePart, time: timeG } }, $set: { inOrOut: "In" } },
        { new: true }
      );
      ...
      const guestCountIn = updatedTimeIn
      io.emit("guestCountIn", guestCountIn);
      io.emit("guestCount", guestCountIn);
      io.emit("notification", notification);
      ...
      const updatedTimeIn = await Guest.findOneAndUpdate(
        { bookingNumber: bookingNumber }, // Use your criteria to identify the user based on userId
        { $push: { timeOut: { date: datePart, time: timeG } }, $set: { inOrOut: "Out" } },
        { new: true }
      );
      console.log("updatedTimeIn", updatedTimeIn);
      if (details !== "") {
...
const updatedVisitor = asyncHandler(async (req, res) => {
  try {
    const { bookingNumber,  details, inOrOut} = req.body;
    const guestNamesArr = req.body["name[]"];
