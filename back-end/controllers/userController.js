...
const signIn = asyncHandler(async (req, res) => {
  ...
  const user = await User.findOne({
    $or: [{ username: emailAddress }, { emailAddress: emailAddress }],
  })
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    await User.updateOne({ emailAddress }, { $set: { isRegistrationComplete: true,username } })
...
const fetchSecuritys = asyncHandler(async (req, res) => {
  try {
    // Fetch users with type 'Security'
    const securitys = await User.find({ type: 'security' });

    return res.status(200).json(securitys);
  } catch (error) {
    return res.status(500).json({ error: "Failed to fetch Security users" });
  }
});

const fetchSecurity = asyncHandler(async (req, res) => {
  const { id } = req.body;
  try {
    var security;

    // Return all the requests
    security = await User.findById({ _id: id });
...
export {
  ...
  editProfileResident,
  deactivateResident,
  deactivateWorker, editProfileWorker,fetchSecuritys,fetchSecurity
};
//working
// import asyncHandler from "express-async-handler";
// import bcrypt from "bcryptjs";
